...
export default function CreateCoursePage() {
  ...
  // Modify the addLesson function to include the new property
  const addLesson = () => {
    setLessons((prev) => [
      ...prev,
      {
...
const onSubmit = async (data) => {
  ...
  try {
    ...
    // Step 1: Handle Thumbnail (no changes)
    ...
    // Step 2: Create Course (no changes)
    const coursePayload = { /* ... */ };
    ...
    // Step 3: Create Chapters using the imported API functions
    ...
    toast.success("Course and all content created successfully!");
...
// import useAuthStore from "../store/useAuthStore";
// import ImagePicker from "./Imagepicker";
// import { coursesAPI, chaptersAPI, uploadsAPI, authAPI } from "../services/api";
...
// // Wrapper component for routing to handle props from a parent page (like a dashboard)
// export function CreateCoursePageRoute() {
//   const location = useLocation();
//   const instructors = location.state?.instructors || [];
//   return <CreateCoursePage instructors={instructors} />;
// }

// // Main Component
// export default function CreateCoursePage({ instructors = [] }) {
//   const navigate = useNavigate();
...
//   // Set default instructor from props
//   useEffect(() => {
...
//   const validateBeforeSubmit = () => {
//     // Validation logic for chapters and quizzes
...
//       let thumbnailUrl = null;
//       if (base64DataUrl) {
//         const upRes = await uploadsAPI.uploadBase64({ dataUrl: base64DataUrl });
//         thumbnailUrl = upRes.url || null;
//       }
...
//         instructorId: (user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN')
...
//           // Add quiz-related data if it's a test
//         };
...
//     instructor: instructors.find(inst => inst.id === selectedInstructorId)?.fullName || user?.fullName || "Instructor",
//     lessons,
//     image: base64DataUrl || "https://via.placeholder.com/400x250?text=Course+Image",
//   };
...
//               <p className="text-gray-600">Share your knowledge and build your community.</p>
//             </div>
//           </div>
//           <div className="flex space-x-4">
//             <Button variant={!showPreview ? "default" : "outline"} onClick={() => setShowPreview(false)}>
//               <BookOpen size={16} className="mr-2" /> Edit Course
//             </Button>
...
//                     label="Course Title"
//                     error={errors.title?.message}
//                     {...register("title", { required: "Course title is required", minLength: { value: 10, message: "Min 10 characters" } })}
//                   />
//                 </div>
//                 <div>
//                   <Input
...
//                 <div className="mt-6">
//                   <label htmlFor="instructor" className="block text-sm font-medium text-gray-700 mb-2">Assign Instructor</label>
//                   <select
//                     id="instructor"
...
//               </div>
//               <ImagePicker onFileAsBase64={setBase64DataUrl} />
//             </div>
...
//                         <input
//                           type="text"
//                           value={lesson.type === 'test' ? lesson.quizTitle : lesson.title}
//                           onChange={(e) => updateLesson(lesson.id, lesson.type === 'test' ? 'quizTitle' : 'title', e.target.value)}
//                           className="w-full px-3 py-2 border border-gray-300 rounded-lg"
//                           placeholder={lesson.type === 'test' ? `Quiz ${idx+1}` :`Chapter ${idx + 1}`}
//                         />
//                       </div>
//                       <div>
...
//                         <Button type="button" variant="outline" onClick={() => removeLesson(lesson.id)} disabled={lessons.length === 1} className="w-full">
//                           <Trash2 size={16} />
//                         </Button>
//                       </div>
//                     </div>
...
//                             className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"
//                           />
//                           {lesson.pdfFile && (<p className="mt-2 text-sm text-gray-600">Selected: {lesson.pdfFile.name}</p>)}
//                         </div>
//                       </div>
//                     ) : (
//                       <div className="mt-4 space-y-6">
//                           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//                               <div className="md:col-span-2">
//                                   {/* Quiz title is now part of the main header */}
//                               </div>
//                               <div>
...
//                                           <Button type="button" variant="outline" onClick={() => removeQuestion(lesson.id, q.id)} disabled={lesson.questions.length === 1}>
//                                               <Trash2 size={14} />
//                                           </Button>
//                                       </div>
//                                       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//                                           <div>
//                                               <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
//                                               <select value={q.type} onChange={(e) => updateQuestion(lesson.id, q.id, "type", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
...
//                                               <input type="text" value={q.text} onChange={(e) => updateQuestion(lesson.id, q.id, "text", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="Enter question" />
//                                           </div>
//                                       </div>
//                                       {(q.type === "single" || q.type === "multiple") && (
//                                           <div className="mt-4">
...
//                                                   <Button type="button" variant="outline" onClick={() => addOption(lesson.id, q.id)}><Plus size={14} className="mr-1" /> Add Option</Button>
//                                               </div>
//                                               <div className="space-y-2">
//                                                   {q.options.map((o) => (
//                                                       <div key={o.id} className="grid grid-cols-[24px_1fr_32px] gap-3 items-center">
//                                                           <input type="checkbox" className="h-4 w-4" checked={o.correct} onChange={(e) => updateOption(lesson.id, q.id, o.id, "correct", e.target.checked)} />
//                                                           <input type="text" value={o.text} onChange={(e) => updateOption(lesson.id, q.id, o.id, "text", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="Option text" />
//                                                           <button type="button" className="text-red-500 hover:text-red-600" onClick={() => removeOption(lesson.id, q.id, o.id)} title="Remove option"><Trash2 size={16} /></button>
//                                                       </div>
//                                                   ))}
//                                               </div>
//                                           </div>
...
//                                               <input type="text" value={q.correctText || ""} onChange={(e) => updateQuestion(lesson.id, q.id, "correctText", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="e.g., 42" />
//                                           </div>
//                                       )}
//                                   </div>
//                               ))}
...
//               <Button type="submit" loading={isLoading} disabled={isLoading}>
//                 <Save size={16} className="mr-2" /> Create Course
//               </Button>
//             </div>
//           </form>
//         ) : (
...
//                 <p className="text-gray-600 text-lg mb-4 whitespace-pre-wrap">{coursePreview.description}</p>
//                 <p className="text-sm text-gray-500">Created by: {coursePreview.instructor}</p>
//               </div>
//               <div className="border-t border-gray-200 pt-6">
