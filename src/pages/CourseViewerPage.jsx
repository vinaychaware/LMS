  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (progress && moduleData.length > 0) {
        const currentMod = moduleData.find(m => m.id === progress.currentModule) || moduleData[0]
        setCurrentModule(currentMod)
        
        const chapterData = await mockAPI.getModuleChapters(currentMod.id)
        setChapters(chapterData)
        
        const currentChap = chapterData.find(c => c.id === progress.currentChapter) || chapterData[0]
      ...
      // Check course test
      const courseTestEligible = mockAPI.checkCourseTestEligibility(user.id, courseId)
          ...
          {studentProgress && (
            <div className="bg-white rounded-lg p-4 border border-gray-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{studentProgress.overallProgress}%</span>
              </div>
              ...
              <Card.Content className="p-0">
                <div className="space-y-2">
                  {modules.map((module, moduleIndex) => {
                    const isModuleCompleted = studentProgress?.completedModules.includes(module.id)
                    const moduleTestResult = studentProgress?.moduleTestResults[module.id]
                      ...
                      <div key={module.id} className="border-b border-gray-100 last:border-b-0">
                        <button
                          ...
                          className={`w-full text-left p-4 hover:bg-gray-50 transition-colors ${
                            currentModule?.id === module.id ? 'bg-primary-50 border-r-2 border-primary-500' : ''
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                isModuleCompleted ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                              }`}>
                                {isModuleCompleted ? <CheckCircle size={16} /> : <span className="text-sm font-medium">{moduleIndex + 1}</span>}
                              </div>
                              <div>
                                <h4 className="text-sm font-medium text-gray-900">{module.title}</h4>
                                <p className="text-xs text-gray-500">{module.totalChapters} chapters</p>
                              </div>
                            </div>
                            {moduleTestResult && (
                              <Badge variant={moduleTestResult.passed ? 'success' : 'danger'} size="sm">
                        ...
                        {currentModule?.id === module.id && (
                          <div className="bg-gray-50">
                            {chapters.map((chapter, chapterIndex) => {
                              const isCompleted = studentProgress?.completedChapters.includes(chapter.id)
                              const isUnlocked = isChapterUnlocked(chapter)
                              
                              return (
                                <button
                                  key={chapter.id}
                                  onClick={() => isUnlocked ? selectChapter(chapter) : toast.error('Complete previous chapter first')}
                                  disabled={!isUnlocked}
                                  className={`w-full text-left p-3 pl-12 hover:bg-gray-100 transition-colors ${
                                    currentChapter?.id === chapter.id ? 'bg-primary-100' : ''
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-xs text-gray-500">{chapter.estimatedDuration}</span>
                                  </div>
                                </button>
                              )
                            })}
                            ...
                            {/* Module Test */}
                            {isModuleTestUnlocked(module.id) && !moduleTestResult?.passed && (
                              <div className="p-3 pl-12 bg-blue-50 border-t border-blue-200">
                                <Button 
                                  size="sm" 
                                  className="w-full"
                                  onClick={() => {
                                    const test = availableTests.find(t => t.moduleId === module.id)
                  ...
                  {/* Course Test */}
                  {isCourseTestUnlocked() && !studentProgress?.courseTestResult?.passed && (
                    <div className="p-4 bg-yellow-50 border-t border-yellow-200">
                      <Button 
                        className="w-full"
                        onClick={() => {
                          const courseTest = availableTests.find(t => t.type === 'course')
                  ...
                  {/* AI Interview */}
                  {isAIInterviewUnlocked() && !studentProgress?.aiInterviewResult && (
                    <div className="p-4 bg-purple-50 border-t border-purple-200">
                      <Button 
                        className="w-full"
                        onClick={() => toast('AI Interview portal coming soon!')}
                      >
            ...
            {currentChapter ? (
              <Card>
                <Card.Header className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge variant="info" size="sm">{currentChapter.estimatedDuration}</Badge>
                    {studentProgress?.completedChapters.includes(currentChapter.id) && (
                ...
                <Card.Content>
                      ...
                      <div className="aspect-video bg-gray-900 rounded-lg overflow-hidden relative">
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="text-center text-white">
                            ...
                            <p className="text-sm opacity-80">Click to play: {currentChapter.title}</p>
                          </div>
                        </div>
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1">
                              <Progress value={videoProgress} size="sm" className="bg-white/20" />
                            </div>
                            <span className="text-white text-sm">
                              {Math.floor(videoProgress * currentChapter.estimatedDuration / 100)} / {currentChapter.estimatedDuration} min
                            </span>
                            <Button 
                              size="sm" 
                              variant="ghost"
                  ...
                  <div className="prose max-w-none mb-6">
                    <div className="bg-gray-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-3">Chapter Content</h3>
                    ...
                    <div className="flex space-x-2">
                      {chapters.findIndex(c => c.id === currentChapter.id) > 0 && (
                        <Button 
                          variant="outline"
                          onClick={() => {
                            const prevIndex = chapters.findIndex(c => c.id === currentChapter.id) - 1
              ...
              <Card>
                <Card.Content className="text-center py-12">
                  <BookOpen size={48} className="mx-auto text-gray-400 mb-4" />
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Select a module to start learning</h3>
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="font-medium text-blue-800">Test Instructions</span>
              </div>
